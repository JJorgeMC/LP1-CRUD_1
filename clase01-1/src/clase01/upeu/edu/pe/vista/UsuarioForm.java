	...
	public UsuarioForm() {
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(txtuser.getText().isEmpty()==false) {
					List<Usuario> lista = udo.readAll();
					int i = 0;
					for(Usuario ls: lista) {
						if(ls.getNomuser().equalsIgnoreCase(txtuser.getText().toString())) {
							tbdatos.getSelectionModel().setSelectionInterval(i, i);
					...
					JOptionPane.showMessageDialog(null, "Error: no ingreso un usuario!!!");
					requestBox();
				}
			}
		});
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(fila>=0) {
					if(txtfiled()) {
						udo.update(fila, new Usuario(txtuser.getText().toString(),txtclave.getText().toString()));					
						borrar();
						limpiar();
						listar();
					}else {
